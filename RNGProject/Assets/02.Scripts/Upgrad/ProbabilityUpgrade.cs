...
public class ProbabilityUpgrade : MonoBehaviour, ISaveManager
{
    ...
    private void Start()
    {
        draw.onClick.AddListener(() => Gacha());
    ...
    public void Gacha()
    {
        ...
        if (result < rareProb)
        {
            Inventory.Instance.RandomRare();
        }
        else if (result < rareProb + uniqueProb)
        {
            Inventory.Instance.RandomUnique();
        }
        else if (result < rareProb + uniqueProb + epicProb)
        {
            Inventory.Instance.RandomEpic();
        }
        else
        {
            Inventory.Instance.RandomLegend();
        }
    }
